...
class EllaRouter:
    ...
    def route_request(self, user_input: str, history: str = "") -> RouteResponse:
        ...
        system_prompt = (
            ...
            "4. Use 'CLOSING' ONLY for farewells, goodbyes, or leaving phrases (bye, goodbye, see you later, take care).\n"
            "5. Use 'BOOKING' for clinic logistics: hours, location, insurance, scheduling, cancellations.\n\n"
            "EXAMPLES:\n"
            "- 'bye' -> CLOSING\n"
            "- 'goodbye' -> CLOSING\n"
            "- 'thank you, see you later' -> CLOSING\n"
            "- 'What are your clinic hours?' -> BOOKING\n"
            "- 'Where is the clinic located?' -> BOOKING\n"
            "- 'My blood pressure is 180/110' -> EMERGENCY (hypertensive crisis)\n"
            "- 'I have chest pain when I breathe' -> TRIAGE (requires assessment)\n"
            "- 'What are the side effects of Lisinopril?' -> GENERAL_INFO (drug info question)\n"
        )
